...
// (1.3) Below is the nested 'ratings' array that contains two other arrays. Destructure the nested 'ratings' arrays into two variables called 'ratings' and 'ratingCount'. In the result of your destructuring, the 'ratings' variables should store a number '4.19', and the 'ratingCount' variable should store a number '144584'.
/**
...
// (2.1) Destructure the first book object from the 'books' array into called 'title', 'author' and 'ISBN'.

const { title, author, ISBN } = books[0];
// console.log(title, author, ISBN);

// (2.2) Each book object has the 'keywords' property. Destructure the first book object from the 'books' array into a variable called tags. The 'tags' variables should be assigned with the value of the 'keywords' property.
...
// If year is undifined(was not passed), it should be assigned with a default value of 'year unknown'.
...
// (3.1) Each book object has the 'author' property, which stores an array of strings(author names) if there are multiple authors, or a single(author name) if there is just one author.
// Declare an array called 'bookAuthors',  and fill it with authors of the first two books from the 'books' array. The 'bookAuthors' array should have just one level (no nested arrays).

const bookAuthors = [...books[0].author, ...books[1].author];
// console.log(bookAuthors);

// (3.2) Write a function called 'spellWord' that accepts a single string as an arrgument. This function should lo to the console each letter of the arguemnt separate by a space.
...
// (4.1) Destructure the 'keyword' property (array) of the first book the 'books' array into variables called 'mainKeyword' and 'rest'. The first keyword should be assigned to 'mainKeyword', and the rest of the keywords should be assigned to the 'rest' variable (it should be an array);
...
/**
 * (5.1) Some of the book have the 'programmingLanguage' property, which specifies what programming language is used in the book, for example
 * 
 * {
  title: 'Algorithms',
  author: ['Robert Sedgewick', 'Kevin Wayne'],
 ...
 *
Write a function called 'hasExampleInJava' that takes a book object from the 'books' arrays argument. This function should return 'true' if the book uses Java, or a string 'no data available' if it uses other language or no programming language at all.
 */
...
/**
 * Some of the object have the 'onlineContent' property, which is either 'true' or 'false'. Loop over the 'books' array, and for the books that provide online content, log to the console a string in this format:`${title} provides online content`. Use short circiting.
 * 
 * {
  title: 'Operating System Concepts',
  // ... removed for clarity
...
// (6.1) There are objects in the 'books' array that don't have the 'onlineContent' property at all. Loop over the 'books' array and log a string to the console in this format: `${title} provides no data about it's online content`.
// for (let i = 0; i < books.length; i++) {
//   books[i].onlineContent ??
//     console.log(
...
// Some of the book object from the 'books' array are missing the 'edition' property. Loop over the 'books' array, and assign this property with a number 1 (if it doesn't already exist). Use logical assignment operators.
// for (let i = 0; i < books.length; i++) {
//   books[i].edition ||= 1;
